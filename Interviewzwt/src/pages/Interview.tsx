...
const Interview = () => {
  ...
  const analyzeResume = async () => {
    ...
    try {
      ...
      // Upload resume to backend for text extraction
      const uploadResp = await apiClient.uploadFile(resumeFile, sid);
      const fileText = uploadResp.extractedText;

      if (!fileText || fileText.trim().length === 0) {
        throw new Error('No text could be extracted from the resume. Please ensure your file contains readable text.');
      }

      const filePath = uploadResp.filePath || uploadResp.fileName || '';

      console.log(`Analyzing resume with ${fileText.length} characters`);

      // analyze using server function
      const analysisResp = await apiClient.analyzeResume({
        resumeText: fileText,
        jobDescription: jobRole?.description || '',
        jobTitle: jobRole?.title || '',
      });
      const score = analysisResp.score ?? 0;
      const feedback = analysisResp.feedback ?? '';

      // update session
      await apiClient.updateSession(sid, {
        resumeScore: score,
